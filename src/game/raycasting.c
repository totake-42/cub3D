 ...
 */
static double	cast_single_ray(t_cub3d *app, double ray_dir_x,
		double ray_dir_y)
{
	int		map_x;
	int		map_y;
...
void	raycast_all(t_cub3d *app)
{
	...
	while (x < app->view->win_width)
	{
		...
		perp_wall_dist = cast_single_ray(app, ray_dir_x, ray_dir_y);
		line_height = (int)(app->view->win_height / perp_wall_dist);
...
void	raycast_single(t_cub3d *app)
{
	...
	distance = cast_single_ray(app, ray_dir_x, ray_dir_y);
	if (frame_count < 5)
	{
